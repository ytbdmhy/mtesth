...
/**
 * ================================================ Discuz! Ucenter API for JAVA ================================================ UC Client 通用调用该类来实现与UC Server之间的通信。 该类实现与UC Server通信的所有接口函数
 * 
 * 更多信息：http://code.google.com/p/discuz-ucenter-api-for-java/ 作者：梁平 (no_ten@163.com) 创建时间：2009-2-20
 */
public class BbsUcClient extends PHPFunctions {
	...
	static {
		InputStream in = BbsUcClient.class.getClassLoader().getResourceAsStream("discuz.properties");
	 ...
	 * @example
	 * 
	 *          $a = authcode('abc', 'ENCODE', 'key'); $b = authcode($a, 'DECODE', 'key'); // $b(abc)
	 * 
	 *          $a = authcode('abc', 'ENCODE', 'key', 3600); $b = authcode('abc', 'DECODE', 'key'); // 在一个小时内，$b(abc)，否则 $b 为空
	 */
	 ...
	 * @param string
	 *            $answer 安全提问答案
	 * @return int -1 : 用户名不合法 -2 : 包含不允许注册的词语 -3 : 用户名已经存在 -4 : email 格式有误 -5 : email 不允许注册 -6 : 该 email 已经被注册 >1 : 表示成功，数值为 UID
	 */
	 ...
	 * @param string
	 *            $answer 安全提问答案
	 * @return int 1 : 修改成功 0 : 没有任何修改 -1 : 旧密码不正确 -4 : email 格式有误 -5 : email 不允许注册 -6 : 该 email 已经被注册 -7 : 没有做任何修改 -8 : 受保护的用户，没有权限修改
	 */
